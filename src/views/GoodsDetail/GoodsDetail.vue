...
const loadGoodsDetail = async () => {
  ...
  const {results} = await request.get("/1.1/classes/goods?where="+JSON.stringify({id:props.id}))
  console.log(results)
  let [data1]=results
  if (!data1.id) {
  ...
  const {results:data2} = await request.get("/1.1/classes/goods_album?where="+JSON.stringify({goods_id:props.id}))
  //({ goods_id: props.id })
