...
const loadGoodList = async () => {
    ...
    const {results} = await request.get("/1.1/classes/goods?where="+JSON.stringify(params)+
        `&limit=${limit}&skip=${skip}`)
    console.log(params)
    console.log(results)
   if (results.length > 0) {
        goodsList.value = goodsList.value.concat(results)
        skip+=limit;
    }else if (skip>0 && results.length<=0) {
        showToast({
            message: '已经到达底部',
            type: 'fail'
        })
